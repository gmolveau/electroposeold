...
<div class="container-fluid">
	...
	<form class="navbar-form navbar-left" role="search" METHOD=GET ACTION="http://vps22975.ovh.net/electropose/index.php?">
    <div id="recherche" class="form-group">
      <input type="text" value="val1" class="form-control" placeholder="Search">
    </div>
...
for ($j = 0; $j <= count($liensc)-1; $j++) {
	if (($j % 2) == 0) {
		...
		$onclickqueue = "SCM.queue({title:\"".$name[$j]."\",url:\"".$liensc[$j]."\"});";
		$onclickplay = "SCM.play({title:\"".$name[$j]."\",url:\"".$liensc[$j]."\"});";
		echo "<div id='EPsound".$j."' class='col-md-6 ".$name[$j]."'></br>"; //créer une div pchq son 
			echo "<div class=' half center-block'>";
				echo "<div class='sound'>";
					echo "<img src=".$artwork[$j]." class='img-rounded center-block' />";
					echo "<div>";
